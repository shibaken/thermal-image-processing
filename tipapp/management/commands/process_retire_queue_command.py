...
Intended to be run from cron, e.g. every minute.
"""
...
class Command(base.BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if count == 0:
            self.stdout.write("No jobs queued for retirement.")
            logger.info("process_retire_queue: no jobs queued.")
            return

        self.stdout.write(f"Found {count} job(s) queued for retirement.")
    ...
    def _retire_job(self, job):
        ...
        self.stdout.write(f"Retiring job {job.id} ({flight_name})...")
        ...
        if errors:
            ...
            self.stdout.write(f"  -> Job {job.id} ({flight_name}) RETIRE_FAILED.")
        else:
            job.status = 'RETIRED'
            job.retired_at = now
            ...
            self.stdout.write(f"  -> Job {job.id} ({flight_name}) RETIRED.")
